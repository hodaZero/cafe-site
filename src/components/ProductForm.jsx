...
export default function ProductForm({ form, setForm, onSubmit, categories, loading }) {
  ...
  const validate = () => {
    const newErrors = {};
    if (!form.name || form.name.trim().length < 2) newErrors.name = "Name min 2 characters";
    if (!form.price || isNaN(form.price) || Number(form.price) <= 0) newErrors.price = "Price must be > 0";
    if (!form.category) newErrors.category = "Category is required";
    if (!form.image && !selectedFile) newErrors.image = "Image is required";
    if (!form.prepTime || isNaN(form.prepTime) || Number(form.prepTime) <= 0) newErrors.prepTime = "Prep time must be > 0";
    if (form.rating === "" || isNaN(form.rating) || Number(form.rating) < 0 || Number(form.rating) > 5) newErrors.rating = "Rating 0-5";
  ...
  return (
    <motion.form
      ...
      className={`${formBg} p-5 md:p-6 rounded-2xl shadow-lg w-full max-w-3xl mx-auto transition-colors duration-300`}
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
        ...
        <div className="col-span-1 md:col-span-2 flex flex-col md:flex-row items-start gap-4 md:gap-5">
          ...
          <div className="flex flex-col items-center gap-2 w-fit mx-auto">
            ...
            <label className={`p-2.5 md:p-3 rounded-lg border cursor-pointer text-center ${inputBg} transition hover:scale-105`}>
        ...
        <div className="col-span-1 md:col-span-2">
          ...
          <textarea
            ...
            className={`w-full p-2.5 md:p-3 rounded-lg border ${inputBg} focus:outline-none focus:ring-2 focus:ring-primary h-24 md:h-28 resize-none transition`}
          />
      ...
      <div className="mt-5 md:mt-6 flex justify-center">
        <motion.button
          ...
          className={`px-6 md:px-8 py-2.5 md:py-3 rounded-xl font-semibold ${btnPrimary} transition-transform`}
