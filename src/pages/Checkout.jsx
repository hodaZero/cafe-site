...
const CheckoutPage = () => {
  ...
  const [orderTypeLocal, setOrderTypeLocal] = useState(selectedTable ? "dineIn" : "");
  const [errors, setErrors] = useState({});
  const [success, setSuccess] = useState("");

  // Fetch table number from state or Firestore
  useEffect(() => {
    const fetchTableNumber = async () => {
      ...
      try {
        const docRef = doc(db, "tables", selectedTable.id || selectedTable);
  ...
  const validate = () => {
    ...
    if (!selectedTable && !orderTypeLocal)
      newErrors.orderType = "Select an order type.";
    setErrors(newErrors);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      await addDoc(collection(db, "users", user.uid, "orders"), {
        ...
        items,
        total,
        orderType: selectedTable || orderTypeLocal === "dineIn" ? "dineIn" : orderTypeLocal,
        tableNumber,
        status: "pending",
        createdAt: new Date(),
      });
  ...
  const bgCard = theme === "light" ? "bg-white text-gray-900 shadow-lg" : "bg-[#1a1a1a] text-white shadow-lg";
  const inputBg = theme === "light" ? "bg-gray-100 text-gray-900" : "bg-[#2a2a2a] text-white placeholder-gray-400";
  const buttonClass = theme === "dark" ? "bg-dark-primary text-dark-text hover:bg-dark-primaryHover" : "bg-light-primary text-white hover:bg-light-primaryHover";
      ...
      <div className={`w-full max-w-6xl rounded-3xl p-10 flex flex-col gap-12 ${bgCard}`}>
        ...
        <div>
          ...
          <div className="flex flex-col gap-5">
            ...
            {items.length === 0 && <p className="text-center opacity-70 text-lg">Your order is empty.</p>}
          </div>
        </div>

        {/* Shipping & Order Type */}
        <div className="flex flex-col gap-6">
          <h2 className="text-3xl font-bold mb-2 text-dark-primary">Enter Shipping Details</h2>
          {[{ icon: <FaUser />, name: "name", placeholder: "Full Name" },
            ...
            { icon: <FaMapMarkerAlt />, name: "address", placeholder: "Shipping Address" }
          ].map((f, i) => (
            <div key={i}>
              <div className={`flex items-center gap-3 p-4 rounded-xl shadow-md ${bgCard}`}>
                {f.icon}
                <input name={f.name} value={form[f.name]} placeholder={f.placeholder} onChange={handleChange} className={`w-full px-3 py-2 rounded-lg focus:outline-none ${inputBg}`} />
              </div>
              {errors[f.name] && <span className="text-red-500 text-sm ml-4">{errors[f.name]}</span>}
            </div>
          ))}
          ...
          <div>
            ...
            {errors.payment && <span className="text-red-500 text-sm ml-4">{errors.payment}</span>}
          </div>

          {/* Order Type */}
          <div className="flex flex-col gap-3 mb-6">
            {(selectedTable || orderTypeLocal === "dineIn") ? (
              <div>
                <span className={`px-4 py-2 rounded-xl ${!tableNumber ? "bg-yellow-400" : "bg-green-500"} text-white font-semibold`}>
                  Table Number: {tableNumber || "Loading..."}
                </span>
              </div>
            ) : (
              <select
            ...
            {errors.orderType && (
              <span className="text-red-500 text-sm ml-4">{errors.orderType}</span>
            )}
          </div>
        </div>
        ...
        <div className={`rounded-3xl p-6 shadow-xl flex flex-col gap-6 ${bgCard}`}>
          ...
          <button onClick={handlePlaceOrder} className={`mt-4 w-full font-bold py-4 rounded-2xl shadow-lg ${buttonClass}`}>
            Confirm Order
          </button>
        </div>
      </div>
